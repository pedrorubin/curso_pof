...
O primeiro é DOMICILIO, que contém dados sobre todos os domicílios da amostra, como água corrente, coleta de lixo e energia elétrica. Nesse registro, cada linha é um domicílio (o total de linhas é o total de domicílios na amostra da POF). 
O segundo é MORADOR, que contém dados de todas as pessoas da amostra, como idade e anos de estudo. Nesse registro, cada linha é uma pessoa, e o total de linhas é o total de pessoas na amostra da POF.
...
A função `str_c` agrupa conjuntos de caracteres.
...
Como dito na primeira aula, as informações da POF se distribuem ao longo de 24 meses, sendo estabelecido o dia 15 de janeiro de 2018 como data de referência. Assim, todos os valores precisam ser deflacionados para representarem valores médios desta data. O IBGE já disponibiliza os valores deflacionados: são as variáveis que olhamos acima e que estão na tabela despesa.
...
Já vimos como ler os microdados e até ja criamos arquivos *.rds*. Em um momento futuro, podemos salvar ainda mais tempo e espaço criando arquivos *.rds* de microdados apenas com as colunas relevantes para a análise de despesa, mas isso fica na conta de cada um.
...
Vamos checar se todos têm 5 dígitos:
...
Na próxima aula vamos replicar a [tabela 6715 do SIDRA](https://sidra.ibge.gov.br/tabela/6715).